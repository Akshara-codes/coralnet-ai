    ...
    <div className="min-h-screen flex items-center justify-center px-6 py-12">
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <motion.div className="flex items-center justify-between text-sm" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.7 }}>
        ...
        <motion.div className="mt-8 text-center" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.9 }}>
          <p className="text-muted-foreground">
            Don't have an account?{' '}
            <Link to="/signup" className="text-primary hover:text-primary-glow transition-colors font-medium">Sign up for research access</Link>
          </p>
        </motion.div>

        <div className="absolute inset-0 pointer-events-none overflow-hidden rounded-2xl">
          {Array.from({ length: 5 }).map((_, i) => (
            <motion.div key={i} className="absolute w-2 h-2 bg-primary/30 rounded-full" style={{ left: `${20 + i * 20}%`, top: '100%' }} animate={{ y: [-20, -400], opacity: [0, 0.6, 0], scale: [0.5, 1, 0.5] }} transition={{ duration: 4 + i, delay: i * 0.8, repeat: Infinity, ease: "easeInOut" }} />
          ))}
        </div>
      </motion.div>
    </div>
